...
cookies = { # get using https://curlconverter.com/
    }

headers = {
    # get using https://curlconverter.com/ 
    }
...
with open('course_data.csv', 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['Field', 'Level', 'Unique', 'Days', 'Hour', 'Room', 'Instruction Mode', 'Instructor', 'Status', 'Flags', 'Core', 'Title', 'Description']
        ...
        for level in levels:
            ...
            while True:
                ...
                for course in courses:
                    ...
                    if len(cells) >= 9 or (len(cells) >= 8 and level == 'G'):
                        ...
                        core = [core.get_text(strip=True) for core in cells[8].select('li')] if (level != 'G') else ''
                        ...
                        title = course_soup.select_one('#details h2').get_text(strip=True)
